  ...
  async function fetchTrunks() {
    try {
      ...
      if (res.ok) {
        const data = await res.json();
        trunks = data.trunks || [];
        if (trunks.length > 0) newTrunkId = trunks[0].ID;
      }
    } catch (e) {}
  }
...
<div class="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700 ease-out max-w-7xl mx-auto w-full">
  ...
  {#if routes.length === 0}
    <div class="p-8 text-center text-slate-500">No outbound routes configured.</div>
  {/if}
...
<Modal bind:isOpen={isModalOpen} title="Create Outbound Route">
  <form on:submit={handleCreateRoute} class="space-y-4">
    <div>
      <label class="block text-sm font-bold text-slate-500 mb-1">Dial Pattern</label>
      <input type="text" bind:value={newPattern} required placeholder="e.g. ^0[1-9]\d{8,9}$" class="w-full bg-white border border-slate-200 rounded-xl px-4 py-2 text-slate-900 focus:ring-indigo-500 focus:border-indigo-500" />
    </div>
    <div>
      <label class="block text-sm font-bold text-slate-500 mb-1">Select Trunk</label>
      <select bind:value={newTrunkId} required class="w-full bg-white border border-slate-200 rounded-xl px-4 py-2 text-slate-900 focus:ring-indigo-500 focus:border-indigo-500">
    ...
    <div>
      <label class="block text-sm font-bold text-slate-500 mb-1">Priority</label>
      <input type="number" bind:value={newPriority} required min="1" max="100" class="w-full bg-white border border-slate-200 rounded-xl px-4 py-2 text-slate-900 focus:ring-indigo-500 focus:border-indigo-500" />
    </div>
