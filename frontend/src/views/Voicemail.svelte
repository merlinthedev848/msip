...
<div class="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700 ease-out max-w-7xl mx-auto w-full">
  <header class="flex justify-between items-end mb-8">
    <div>
      <h1 class="text-3xl font-bold tracking-tight text-white mb-1">Visual Voicemail</h1>
      <p class="text-gray-400 text-sm">Listen and read transcriptions of incoming messages.</p>
    </div>
  </header>

  <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
    {#if voicemails.length === 0}
      <div class="col-span-1 md:col-span-2 p-8 text-center text-gray-400">No voicemails found.</div>
    {/if}
    ...
    <GlassCard hoverEffect={true}>
      <div class="flex justify-between items-start mb-4">
        <div class="flex items-center space-x-3">
          ...
          <div>
            <p class="font-semibold text-white">{vm.CallerID}</p>
            <p class="text-xs text-gray-500">Extension: {vm.Extension} • Duration: {vm.Duration}s</p>
          </div>
        </div>
        {#if !vm.IsRead}
      ...
      <div class="bg-gray-900/50 p-4 rounded-lg border border-gray-800 mb-4">
        <p class="text-sm text-gray-300 italic font-serif leading-relaxed">[Voicemail transcription unavailable]</p>
      </div>
      ...
      <div class="flex items-center space-x-4">
        ...
        <span class="text-xs text-gray-500 font-mono">0:00 / {vm.Duration}</span>
      </div>
    </GlassCard>
    {/each}
  </div>
