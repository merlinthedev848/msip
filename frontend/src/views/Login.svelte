...
<div class="min-h-screen bg-[#0B0F19] flex items-center justify-center p-4">
  <div class="max-w-md w-full bg-gray-900/50 backdrop-blur-xl border border-gray-800 rounded-2xl shadow-2xl p-8 relative overflow-hidden">
    ...
    <div class="relative z-10">
      <div class="text-center mb-10">
        ...
        <h1 class="text-3xl font-bold tracking-tight text-white mb-2">MSIP PBX</h1>
        <p class="text-gray-400 text-sm">Enterprise Unified Communications</p>
      </div>
      ...
      <form on:submit={handleLogin} class="space-y-5">
        ...
        <div>
          ...
          <input id="email" type="email" bind:value={email} required placeholder="admin@pbx.local" class="w-full bg-gray-950/50 border border-gray-800 rounded-xl px-4 py-3 text-white focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all outline-none">
        </div>
        
        <div>
          <label for="password" class="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Password</label>
          <input id="password" type="password" bind:value={password} required placeholder="••••••••" class="w-full bg-gray-950/50 border border-gray-800 rounded-xl px-4 py-3 text-white focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all outline-none">
        </div>
        
        <button type="submit" disabled={isLoading} class="w-full bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 text-white py-3.5 rounded-xl font-bold transition-all shadow-lg shadow-indigo-600/20 disabled:opacity-50 mt-4">
